...
namespace FoxTunes
{
    ...
    public class BassDsdMemoryLoadingBehaviour : StandardBehaviour, IDisposable
    {
        public ConcurrentDictionary<string, LoadingTask> Tasks { get; private set; }

        public BassMemoryProgressHandler Handler;

        public BassDsdMemoryLoadingBehaviour()
        {
        ...
        protected virtual void OnIsLoadedChanged(object sender, EventArgs e)
        {
            if (this.BassLoader.IsLoaded)
            {
                try
                {
                    BassMemory.Dsd.Progress(this.Handler);
        ...
        {
            public const string ID = "718BF7A1-A708-4C55-A50C-3467BAB05D57";
